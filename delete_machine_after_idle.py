...
p = argparse.ArgumentParser(
    description="Run this script on a machine in the background to delete the machine when load stays below some threshold for some number of mintues", 
...
p.add_argument("--minutes", type=float, default=60, help="delete machine after idle for this many minutes")
args = p.parse_args()

idle_minutes_counter = 0
logging.info("This machine will be deleted if load stays <= {} for {} minutes...".format(args.load_threshold, args.minutes))
...
while idle_minutes_counter < args.minutes:
    ...
    if current_load <= args.load_threshold:
        ...
        logging.info("Current load = {}. This machine will be deleted if load stays <= {} for {:0.1f} more minutes.".format(
...
logging.info("Deleting machine...")
command = ["sudo", "delete this machine"]
